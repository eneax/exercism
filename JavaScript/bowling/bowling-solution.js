...
export class Bowling {
  ...
  get gameOver() {
    return (
      this.isLastFrame() &&
      !this.inBonus &&
      Bowling.isFullFrame(last(this.#frames))
    );
  }
  ...
  verifyRoll(pins) {
    ...
    if (
      [
        ...
        (roll) =>
          this.inBonus &&
          !Bowling.isStrike(this.#bonus) &&
          sumOf(this.#bonus.concat(roll)),
      ].some((count) => count(pins) > Bowling.PINS)
    )
  ...
  score() {
    ...
    let accumulate = 0;
    return (
      this.#frames.reduce((score, frame, i) => {
        score +=
          sumOf(frame) +
          frame.reduce((bonus, roll) => {
            if (accumulate) {
        ...
        if (this.isLastFrame(i + 1)) score += accumulate && this.#bonus[0];
          ...
          accumulate +=
            (Bowling.isSpare(frame) && 1) || (Bowling.isStrike(frame) && 2);
