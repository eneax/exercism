export class QueenAttack {
  ...
  static isOnBoard(coordinates) {
    return coordinates.every(
      (coordinate) =>
        coordinate >= 0 && coordinate < QueenAttack.BOARD_DIMENSION
    );
  }

  #white;
  #black;
  #board = Array.from({ length: QueenAttack.BOARD_DIMENSION }, () =>
    Array(QueenAttack.BOARD_DIMENSION).fill("_")
  );

  constructor({ white = [7, 3], black = [0, 3] } = {}) {
    if (white.every((_, i) => white[i] === black[i])) {
      throw new Error("Queens cannot share the same space");
    }

    if (![white, black].every(QueenAttack.isOnBoard)) {
